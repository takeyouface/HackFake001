    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if cached_filepath != filepath:
                ...
                if image is None:
                    self.log_err (f'无法打开 {filepath}, 原因: cv2_imread() fail.')
                    return data
...
class DeletedFilesSearcherSubprocessor(Subprocessor):
    ...
    #override
    def on_clients_initialized(self):
        io.progress_bar ("正在扫描被删除的文件", len (self.input_paths))
...
def main(detector=None,
    ...
    if not manual_output_debug_fix and len(output_images_paths) > 0:
        if len(output_images_paths) > 128:
            continue_extraction = io.input_bool ("是否继续上次未完成的人脸提取?", True, help_message="Extraction can be continued, but you must specify the same options again.")
        ...
        if len(output_images_paths) > 128 and continue_extraction:
            try:
                ...
                io.log_err("在获取最后一个索引时出错,提取不能继续.")
                return
    ...
    if face_type is None:
        face_type = io.input_str ("选择人脸训练范围,全脸<f>/整张脸<wf>/头部<head>", 'wf', ['f','wf','head'], help_message="Full face / whole face / head. 'Whole face' covers full area of face include forehead. 'head' covers full head, but requires XSeg for src and dst faceset.").lower()
        face_type = {'f'  : FaceType.FULL,
                     'wf' : FaceType.WHOLE_FACE,
                     'head' : FaceType.HEAD}[face_type]

    if max_faces_from_image is None:
        max_faces_from_image = io.input_int(f"设置每帧中提取人脸的最大值<默认为0，提取所有人脸！>", 0, help_message="If you extract a src faceset that has frames with a large number of faces, it is advisable to set max faces to 3 to speed up extraction. 0 - unlimited")

    if image_size is None:
        image_size = io.input_int(f"设定提取人脸尺寸<如果你觉得图像够清晰且不需要提升，可以设定大于512，否则默认！>", 512 if face_type < FaceType.HEAD else 768, valid_range=[256,2048], help_message="Output image size. The higher image size, the worse face-enhancer works. Use higher than 512 value only if the source image is sharp enough and the face does not need to be enhanced.")

    if jpeg_quality is None:
        jpeg_quality = io.input_int(f"设定提取人脸图像质量<默认为100，如果你喜欢烂画质可以调低！>", 100, valid_range=[1,100], help_message="Jpeg quality. The higher jpeg quality the larger the output file size.")
    ...
    if output_debug is None:
        output_debug = io.input_bool (f"写入调试图像到 {output_debug_path.name}?", False)
    ...
    if manual_output_debug_fix:
        if not output_debug_path.exists():
            io.log_err(f'{output_debug_path} 没有找到. 使用“写调试图像”选项重新提取人脸.')
            return
        else:
            detector = 'manual'
            ...
            io.log_info('发现 %d 张图片.' % (len(input_image_paths)))
    else:
