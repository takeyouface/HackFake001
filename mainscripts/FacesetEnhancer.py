...
def process_folder ( dirpath, cpu_only=False, force_gpu_idxs=None ):
    ...
    io.log_info (f"从 {dirpath_parts} 提升人脸细节")
    io.log_info ( f"处理中 to {output_dirpath_parts}")
    ...
    if is_merge:
        io.log_info (f"移动提升细节后的人脸到 {dirpath_parts}")
        ...
        io.log_info (f"删除中 {output_dirpath_parts}")
