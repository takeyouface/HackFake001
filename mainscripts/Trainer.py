...
def trainerThread (s2c, c2s, e,
    ...
    while True:
        try:
            ...
            model = mode1s.import_model(model_class_name)(
                        is_training=True,
            ...
            for i in itertools.count(0,1):
                if not debug:
                    ...
                    if not is_reached_goal:
                        ...
                        if model.get_iter() == 0:
                            io.log_info("")
                            io.log_info("尝试进行第一次迭代，如果发生错误，请降低模型参数。")
                            io.log_info("")
                        ...
                        if model.get_target_iter() != 0 and model.is_reached_iter_goal():
                            io.log_info ('完成目标跌代数量.')
                            model_save()
                            is_reached_goal = True
                            io.log_info ('您现在可以使用预览.')
...
def main(**kwargs):
    ...
    if no_preview:
        ...
        wnd_name = "Training preview | HackFake"
        ...
        while True:
            ...
            if update_preview:
                ...
                # HEAD
                head_lines = [
                    '[s]:保存并继续 [b]:备份模型<XSeg模型无效> [enter]:保存并退出',
                    '[p]:强制刷新预览 [space]:下一个预览 [l]:修改曲线范围',
                ...
                if loss_history is not None:
                    ...
                    lh_img = mode1s.ModelBase.get_loss_history_preview(loss_history_to_show, iter, w, c)
