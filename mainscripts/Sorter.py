...
class BlurEstimatorSubprocessor(Subprocessor):
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if dflimg is None or not dflimg.has_data():
                self.log_err (f"{filepath.name} 不符合DFL使用图像标准")
    ...
    def process_info_generator(self):
        ...
        io.log_info(f'运行设备 {cpu_count} CPUs')
...
class HistSsimSubprocessor(Subprocessor):
    ...
    def process_info_generator(self):
        ...
        io.log_info(f'运行设备 {cpu_count} threads')
        for i in range(cpu_count):
            yield 'CPU%d' % (i), {'i':i}, {}
    ...
    #override
    def on_data_return (self, host_dict, data):
        raise Exception("无法处理数据,请减少图像数量,然后重试.")
...
class FinalLoaderSubprocessor(Subprocessor):
    ...
    #override
    def on_clients_initialized(self):
        io.progress_bar ("加载中", len (self.img_list))
...
class FinalHistDissimSubprocessor(Subprocessor):
    ...
    #override
    def on_clients_initialized(self):
        io.progress_bar ("按历史异议排序", len(self.pitch_yaw_sample_list_idxs) )
...
def sort_best(input_path, faster=False):
    target_count = io.input_int ("设置保留人脸数量?", 2000)

    io.log_info ("执行最佳人脸排序中.")
    if faster:
        io.log_info("使用更快的算法,人脸将按源矩形区域而不是模糊进行排序.")
    ...
    for g in io.progress_bar_generator ( range(grads), "按侧脸角度排序"):
        yaw = grads_space[g]
    ...
    for g in io.progress_bar_generator ( range (grads), "按模糊度排序"):
        img_list = yaws_sample_list[g]
        if img_list is None:
            continue
    ...
    for g in io.progress_bar_generator ( range (grads), "按俯仰角度排序"):
        img_list = yaws_sample_list[g]
        if img_list is None:
            continue
    ...
    for g in io.progress_bar_generator (range (grads), "挑选最好的"):
...
def sort_by_absdiff(input_path):
    io.log_info ("排序中<按绝对差>...")

    is_sim = io.input_bool ("按相似项排序?", True, help_message="否则按不同的排序.")
    ...
    next_id = 0
    sorted = [next_id]
    for i in io.progress_bar_generator ( range(image_paths_len-1), "排序中" ):
...
def final_process(input_path, img_list, trash_img_list):
    if len(trash_img_list) != 0:
        ...
        io.log_info ("将 %d 个项目筛选到 %s" % ( len(trash_img_list), str(trash_path) ) )

        for filename in pathex.get_image_paths(trash_path):
            Path(filename).unlink()

        for i in io.progress_bar_generator( range(len(trash_img_list)), "移动丢弃中", leave=False):
    ...
    if len(img_list) != 0:
        for i in io.progress_bar_generator( [*range(len(img_list))], "重命名中", leave=False):
        ...
        for i in io.progress_bar_generator( [*range(len(img_list))], "重命名中"):
            src = Path (img_list[i][0])
...
sort_func_methods = {
    'blur':        ("按模糊度", sort_by_blur),
    'motion-blur': ("按运动模糊", sort_by_motion_blur),
    'face-yaw':    ("按侧脸方向", sort_by_face_yaw),
    'face-pitch':  ("按俯仰角度", sort_by_face_pitch),
    'face-source-rect-size' : ("按源图像中的人脸大小", sort_by_face_source_rect_size),
    'hist':        ("按直方图相似性", sort_by_hist),
    'hist-dissim': ("按直方图差异性", sort_by_hist_dissim),
    'brightness':  ("按明亮度", sort_by_brightness),
    'hue':         ("按色调", sort_by_hue),
    'black':       ("按图像中黑色像素点数量", sort_by_black),
    'origname':    ("按原始文件名顺序<不恢复原始文件名>", sort_by_origname),
    'oneface':     ("按图像中的人脸数量", sort_by_oneface_in_image),
    'absdiff':     ("按绝对像素差", sort_by_absdiff),
    'final':       ("按最佳人脸", sort_best),
    'final-fast':  ("按最佳人脸<快速>", sort_best_faster),
}

def main (input_path, sort_by_method=None):
    io.log_info ("排序工具箱运行中.\r\n")

    if sort_by_method is None:
        io.log_info(f"选择排序方式:")
