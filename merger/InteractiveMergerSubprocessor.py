...
class InteractiveMergerSubprocessor(Subprocessor):
        ...
        def process_data(self, pf): #pf=ProcessingFrame
            ...
            if len(frame_info.landmarks_list) == 0:
                ...
                if cfg.mode == 'raw-predict':        
                    ...
                    self.log_info (f'{filepath.name}, 未发现人脸数据, 复制图片并跳过！')
    ...
    #override
    def __init__(self, is_interactive, merger_session_filepath, predictor_func, predictor_input_shape, face_enhancer_func, fanseg_full_face_256_extract_func, xseg_256_extract_func, merger_config, frames, frames_root_path, output_path, output_mask_path, model_iter, subprocess_count=4):
        if len (frames) == 0:
        ...
        if self.is_interactive and self.merger_session_filepath.exists():
            ...
            if io.input_bool ("使用上次保存的合并参数？", True):
                try:
        ...
        if self.is_interactive and session_data is not None:
            ...
            if frames_equal:
                io.log_info ('使用上次保存好的合并参数来至 ' + '/'.join (self.merger_session_filepath.parts[-2:]) )
    ...
    def on_clients_initialized(self):
        io.progress_bar ("图像合成进度", len(self.frames_idxs)+len(self.frames_done_idxs), initial=len(self.frames_done_idxs) )
    ...
    def on_clients_finalized(self):
        ...
        if self.is_interactive:
            ...
            io.log_info ("合并参数已经保存到 " + '/'.join (self.merger_session_filepath.parts[-2:]) )
