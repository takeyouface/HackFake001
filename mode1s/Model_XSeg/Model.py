...
class XSegModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if not self.is_first_run() and ask_override:
            if io.input_bool(f"重置CA权重并重练模型？", False, help_message="Reset model weights and start training from scratch."):
                self.set_iter(0)

        default_face_type          = self.options['face_type']          = self.load_or_def_option('face_type', 'wf')

        if self.is_first_run():
            self.options['face_type'] = io.input_str ("选择人脸训练范围,半脸<h>/中脸<mf>/全脸<f>/整张脸<wf>/头部<head>", default_face_type, ['h','mf','f','wf','head'], help_message="Half / mid face / full face / whole face / head. Choose the same as your deepfake model.").lower()

        if self.is_first_run() or ask_override:
            self.ask_batch_size(4, range=[2,256])
