...
class ModelBase(object):
    def __init__(self, is_training=False,
            ...
            if force_model_name is not None:
                ...
                while True:
                    ...
                    for filepath in pathex.get_file_paths(saved_models_path):
                        ...
                        if filepath_name.endswith(f'{model_class_name}_info.001'):
                    ...
                    if len(saved_models_names) != 0:
                        if silent_start:
                            ...
                            io.log_info(f'静默启动: 已经选择模型 "{self.model_name}"')
                        else:
                            io.log_info ("选择一个模型序号，或者输入一个名称建立一个新模型！")
                            io.log_info ("[r] : 重命名")
                            io.log_info ("[d] : 删除")
                            io.log_info ("")
                                ...
                                if len(inp) == 1:
                                    ...
                                    if is_rename or is_delete:
                                        if len(saved_models_names) != 0:
                                            ...
                                            if is_rename:
                                                name = io.input_str(f"输入一个你想重命名的模型名称！")
                                            elif is_delete:
                                                name = io.input_str(f"输入一个你想删除的模型名称！")
                        ...
                        io.log_info (f"未发现模型文件，请确保模型文件放置正确！")
                        sys.exit()
                    break
        ...
        model_data = {}
        self.model_data_path = Path( self.get_strpath_storage_for_file('info.001') )
        if self.model_data_path.exists():
            io.log_info (f"加载 {self.model_name} 模型中...")
    ...
    def update_sample_for_preview(self, choose_preview_history=False, force_new=False):
        if self.sample_for_preview is None or choose_preview_history or force_new:
            if choose_preview_history and io.is_support_windows():
                wnd_name = "[p] - 下一个预览. [space] - 切换预览类型. [enter] - 确认."
                io.log_info (f"选择图像作为预览历史记录. {wnd_name}")
    ...
    def ask_override(self):
        return self.is_training and self.iter != 0 and io.input_in_time ("您可以在5秒内按回车键<ENTER>调整模型参数!!!", 5 if io.is_colab() else 5 )
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options['autobackup_hour'] = io.input_int(f"设置模型自动备份间隔时间<1-24小时>", default_autobackup_hour, add_info="0..24", help_message="Autobackup model files with preview every N hour. Latest backup located in model/<>_autobackups/01")
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        self.options['write_preview_history'] = io.input_bool(f"是否写入预览历史图片,保存位置<模型名称>_history", default_write_preview_history, help_message="Preview history will be writed to <ModelName>_history folder.")
    ...
    def ask_target_iter(self, default_value=0):
        ...
        self.options['target_iter'] = max(0, io.input_int("设置目标迭代数", default_target_iter))

    def ask_random_flip(self):
        default_random_flip = self.load_or_def_option('random_flip', True)
        self.options['random_flip'] = io.input_bool("是否启用人脸随机翻转,解决SRC比DST角度少合成慢的问题", default_random_flip, help_message="Predicted face will look more naturally without this option, but src faceset should cover all face directions as dst faceset.")
    ...
    def ask_batch_size(self, suggest_batch_size=None, range=None):
        ...
        batch_size = max(0, io.input_int("同批次训练量，简称BS值", default_batch_size, valid_range=range, help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually."))
    ...
    def get_loss_history_preview(loss_history, iter, w, c):
        ...
        lh_text = '模型迭代次数: %d' % (iter) if iter != 0 else ''
