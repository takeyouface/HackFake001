...
class nn():
    ...
    def initialize(device_config=None, floatx="float32", data_format="NHWC"):
        ...
        if floatx == "float32":
            ...
            raise ValueError(f"不支持的floatx {floatx}")
    ...
    def set_data_format(data_format):
        if data_format != "NHWC" and data_format != "NCHW":
            raise ValueError(f"不支持的data_format {data_format}")
    ...
    def to_data_format( x, to_data_format, from_data_format):
        ...
        if to_data_format == "NHWC":
            ...
            raise ValueError(f"不支持的to_data_format {to_data_format}")
    ...
    def ask_choose_device_idxs(choose_only_one=False, allow_cpu=True, suggest_best_multi_gpu=False, suggest_all_gpu=False):
        ...
        io.log_info ("")
        if choose_only_one:
            io.log_info ("选择一个显卡序号!")
        else:
            io.log_info ("选择一个或多个显卡序号(用“,”分隔,如：0,1)!")
        io.log_info ("")
        ...
        while True:
            try:
                if choose_only_one:
                    choosed_idxs = io.input_str("选择哪个显卡序号?", best_device_indexes)
                else:
                    choosed_idxs = io.input_str("选择哪些显卡序号?", best_device_indexes)
