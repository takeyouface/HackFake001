...
class SampleGeneratorFacePerson(SampleGeneratorBase):
    def __init__ (self, samples_path, debug=False, batch_size=1,
        ...
        raise NotImplementedError("当前未实现SampleGeneratorFacePerson!")
        ...
        while True:
            ...
            for n_batch in range(bs):
                ...
                try:
                    ...
                    raise Exception ("样本中发现异常 %s. 错误: %s" % (sample.filename, traceback.format_exc() ) )
                ...
                if self.person_id_mode==1:
                    ...
                    if sample1 is not None and sample2 is not None:
                        try:
                            ...
                            raise Exception ("样本中发现异常 %s. 错误: %s" % (sample1.filename, traceback.format_exc() ) )

                        try:
                            x2, = SampleProcessor.process ([sample2], self.sample_process_options, self.output_sample_types, self.debug)
                        except:
                            raise Exception ("样本中发现异常 %s. 错误: %s" % (sample2.filename, traceback.format_exc() ) )
