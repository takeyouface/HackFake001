...
class SampleLoader:
    ...
    def get_person_id_max_count(samples_path):
        ...
        try:
            ...
            io.log_err(f"加载samplelib.PackedFaceset.load时出错 {str(samples_dat_path)}, {traceback.format_exc()}")

        if samples is None:
            raise ValueError("未找到打好包的人脸素材数据集！")
        persons_name_idxs = {}
        ...
        if            sample_type == SampleType.IMAGE:
            ...
            if  samples[sample_type] is None:
                ...
                if result is not None:
                    io.log_info (f"从 {samples_path} 数据集中加载了 {len(result)} 张人脸素材！")
...
class FaceSamplesLoaderSubprocessor(Subprocessor):
    ...
    def on_clients_initialized(self):
        io.progress_bar ("加载人脸数据中", len (self.image_paths))
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if dflimg is None or not dflimg.has_data():
                self.log_err (f"人脸素材加载器: {filename} 不符合DFL使用图像标准.")
                data = None
            else:
