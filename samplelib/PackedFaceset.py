...
class PackedFaceset():
    ...
    def pack(samples_path):
        ...
        if samples_dat_path.exists():
            ...
            io.input("按回车键<ENTER>覆盖并继续！")

        as_person_faceset = False
        dir_names = pathex.get_all_dir_names(samples_path)
        if len(dir_names) != 0:
            as_person_faceset = io.input_bool(f"{len(dir_names)} 找到子目录，作为人脸数据处理？", True)
        ...
        for filename in io.progress_bar_generator(image_paths, "删除文件中"):
            Path(filename).unlink()

        if as_person_faceset:
            for dir_name in io.progress_bar_generator(dir_names, "删除文件夹中"):
    ...
    def unpack(samples_path):
        ...
        if not samples_dat_path.exists():
            io.log_info(f"{samples_dat_path} : 文件未找到！")
            return
